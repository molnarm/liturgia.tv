...
class QueryTests(TestCase):
    def setUp(self):
        ...
        self.liturgy1 = Liturgy.objects.create(name='Liturgy 1', denomination=self.denomination1)
        self.liturgy2 = Liturgy.objects.create(name='Liturgy 2', denomination=self.denomination2)

        for location in Location.objects.all():
            for liturgy in Liturgy.objects.all():
                Event.objects.create(location=location, liturgy=liturgy, name=location.name + ' ' + liturgy.name)
    ...
    def test_query_city_location_filter(self):
        schedule = queries.get_schedule(city_slug='city-1', location_slug='church-a')
    ...
    def test_query_denomination_city_filter(self):
        schedule = queries.get_schedule(denomination_slug='denomination-1', city_slug='city-2')
    ...
    def test_validity(self):
        ...
        schedule11.save()
        schedule12 = EventSchedule.objects.get(event__location=self.location1, event__liturgy=self.liturgy2)
    ...
    def test_is_active(self):
        ...
        self.location1.save()
        event21 = Event.objects.get(location=self.location2, liturgy=self.liturgy1)
    ...
    def __assert_schedule(self, expected, schedule):
        ...
        for expected_item, actual_item in zip(expected, schedule):
            self.assertEqual(expected_item, (actual_item.name, actual_item.city_name))